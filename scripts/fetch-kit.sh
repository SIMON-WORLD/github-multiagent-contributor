...
#   # or: curl -fsSL -o /tmp/fetch-kit.sh https://raw.githubusercontent.com/academic-door/github-agent-contributor-workflow/main/scripts/fetch-kit.sh && bash /tmp/fetch-kit.sh
#
# Optional: pass another source repo as $1, e.g. bash fetch-kit.sh owner/repo
set -euo pipefail

SRC="${1:-academic-door/github-agent-contributor-workflow}"
